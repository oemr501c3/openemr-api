...
function createToken($userId, $create = true, $device_token = '') {
    ...
    if ($create) {
        $strQuery = "INSERT INTO tokens VALUES('', " . $userId . " ,'" . $token . "','" . $device_token . "' ,'" . date('Y-m-d h:i:s') . "', '',0,0,0,0)";
        $result = $db->query($strQuery);
    } else {
        $strQuery = "UPDATE `tokens` SET `token` = '$token' ";
        if ($device_token) {
...
function validateToken($token) {
    global $db;
    $query = "SELECT * FROM tokens WHERE token LIKE '{$token}' AND (expire_datetime = 0 OR expire_datetime >= NOW())";
...
function getUsername($userId) {
    global $db;
    $strQuery = "SELECT username FROM medmasterusers WHERE id=" . $userId;
...
function getDeviceToken($username) {
    ...
    $strQuery = "SELECT t.device_token
                        ...
                        INNER JOIN `tokens` AS t ON mu.id = t.user_id
...
function updateBadge($token, $update_badge_type) {
    ...
    $query = "UPDATE `tokens` SET {$update} WHERE token = '$token'";
    $result = $db->query($query);
    return $result;
}
...
function getSecretkey($userId) {
    global $db;
    $strQuery = "SELECT secret_key FROM medmasterusers WHERE id = " . $userId;
    $result = $db->get_row($strQuery);
    if ($result) {
